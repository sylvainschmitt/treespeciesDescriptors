...
paracou %>% 
  ...
  vroom::vroom_write("data/species_growth_hab.tsv")
...
t <- paracou %>% 
  ...
  filter(CensusYear < FirstDead) %>% 
  summarise(AGR = (last(DBH) - first(DBH))/(last(CensusYear) - first(CensusYear)),
            RGR = (last(DBH) - first(DBH))/(last(CensusYear) - first(CensusYear))/first(DBH))
t %>% 
  group_by(Genus, Species) %>% 
  gather(variable, value, -Genus, -Species, -idTree) %>% 
  filter(value > 0) %>% 
  mutate(log_value = log(value)) %>% 
  group_by(Genus, Species, variable) %>% 
  summarise(median = median(value), 
            median_log = median(log_value), 
            sd_log = sd(log_value)) %>% 
...
```{r growthtab}
#| message: false
vroom::vroom("data/species_growth_hab.tsv") %>% 
...
```{r}
vroom::vroom("data/species_growth.tsv") %>% 
  ggplot(aes(paste(Genus, Species), median_log)) +
  geom_point() +
  geom_linerange(aes(ymin = median_log-sd_log, ymax = median_log+sd_log)) +
  theme_bw() +
  facet_wrap(~ variable) +
  coord_flip() +
...
vroom::vroom("data/paracou.tsv") %>% 
  ...
  group_by(Genus, Species) %>% 
  summarise(mortality = sum(death) / n() / (max(y_max) - min(y_min)) * 100) %>% 
  vroom::vroom_write("data/species_mortality_0.tsv")
```
...
paracou <- vroom::vroom("data/paracou.tsv") %>% 
  ...
  filter(dbh >= 10) %>% 
  mutate(ba_ind = (dbh/2)^2*pi/10^4/6.25) %>% 
  group_by(CensusYear, Genus, Species) %>% 
  mutate(ba_sp = sum(ba_ind)) %>% 
  group_by(idTree) %>% 
  mutate(recruit_year = min(CensusYear)) %>%
  group_by(Plot) %>% 
  mutate(first_inventory = min(CensusYear)) %>% 
  ungroup() %>% 
  filter(recruit_year != first_inventory) %>% 
  filter(recruit_year == CensusYear)
paracou_time <- paracou %>% 
  select(Plot, CensusYear, Genus, Species) %>% 
  unique() %>% 
  group_by(Plot, Genus, Species) %>% 
  arrange(CensusYear) %>% 
  mutate(year_prev = lag(CensusYear)) %>% 
  mutate(delta_t = CensusYear - year_prev)
paracou %>% 
  left_join(paracou_time) %>% 
  group_by(Genus, Species, CensusYear, ba_sp, delta_t, year_prev) %>% 
  summarise(ba_recruit = sum(ba_ind)) %>% 
  group_by(Genus, Species) %>% 
  arrange(Genus, Species, CensusYear) %>% 
  mutate(ba_recruit_pct = ba_recruit/lag(ba_sp)/delta_t*100) %>% 
  na.omit() %>%
  select(Genus, Species, CensusYear, ba_recruit_pct) %>% 
  write_tsv("data/species_recruit.tsv")
```

```{r}
read_tsv("data/species_recruit.tsv") %>% 
  ggplot(aes(CensusYear, ba_recruit_pct)) +
  geom_line() +
  theme_bw() +
  facet_wrap(~ paste(Genus, Species)) +
  scale_y_log10()
```

```{r}
read_tsv("data/species_recruit.tsv") %>% 
  ggplot(aes(paste(Genus, Species), ba_recruit_pct)) +
  geom_boxplot() +
  theme_bw() +
  scale_y_log10() +
  coord_flip()
