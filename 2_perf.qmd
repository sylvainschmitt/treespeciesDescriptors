...
paracou <- vroom::vroom("data/paracou.tsv") %>% 
  ...
  group_by(Plot, CensusYear, Genus, Species) %>% 
  mutate(ba_sp_plot = sum(ba_ind)) %>% 
  group_by(idTree) %>% 
...
paracou %>% 
  ...
  group_by(Plot, Genus, Species, CensusYear, ba_sp_plot, delta_t, year_prev) %>% 
  summarise(ba_recruit = sum(ba_ind)) %>% 
  group_by(Genus, Species, Plot) %>% 
  arrange(CensusYear) %>% 
  mutate(ba_recruit_pct = ba_recruit/lag(ba_sp_plot)/delta_t) %>% 
  na.omit() %>%
  select(Plot, Genus, Species, CensusYear, ba_recruit_pct) %>% 
  write_tsv("data/species_recruit.tsv")
```

```{r}
read_tsv("data/species_recruit.tsv") %>% 
  ggplot(aes(CensusYear, ba_recruit_pct, col = paste(Genus, Species))) +
  geom_line() +
  facet_wrap(~ Plot) +
  theme_bw() +
  scale_y_log10()
